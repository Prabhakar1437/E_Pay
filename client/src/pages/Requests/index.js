...
import { GetAllRequests, UpdateRequestStatus } from "../../apicalls/requests";
    ...
    const updateStatus = async(record,status) =>{
        try {
            ...
            if(status === "accepted" && record.amount > user.balance){
                message.error("Itne paise hi nahi hai!");
                return;
            }
            else{
                dispatch(ShowLoading());
                ...
                if(record.status === "Pending" && record.receiver._id === user._id){
                    return <div className="flex gap-1">
                        <h1 className="text-sm underline"
                            onClick={()=> updateStatus(record,"accepted")}>Accept</h1>
                        <h1 className="text-sm underline"
                            onClick={()=> updateStatus(record,"rejected")}>Reject</h1>
                    </div>
                }
            }
        }
